...
static bool is_asset_or_sw_path(const char *path) {
    if (!path) return false;
    if (strncmp(path, "/assets/", 8) == 0) return true;
    if (strcmp(path, "/sw.js") == 0 || strncmp(path, "/sw.js?", 7) == 0) return true;
    return false;
}

/* Redirect to the Referer URL only when it is same-origin or a local path,
 * and does not point to a static asset or service worker script.
...
void redirect_referer_safe(cwist_http_response *res, const char *referer, const char *fallback) {
    if (referer && referer[0]) {
        if (referer[0] == '/' && referer[1] != '/') {
            if (!is_asset_or_sw_path(referer)) {

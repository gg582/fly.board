...
void global_middleware(cwist_http_request *req, cwist_http_response *res, cwist_handler_func next) {
    ...
    /* -----------------------------------------------------------------------
     * Middleware-level request coalescing for anonymous GET requests.
     *
     * When the same URL is hit concurrently by many clients (e.g. a burst of
     * identical page loads), only one request executes the handler and DB
     * work; all others wait and receive a copy of the same rendered response.
     *
     * Conditions for coalescing:
     *   - GET or HEAD (idempotent, no side-effects)
     *   - No Cookie header (anonymous / unauthenticated request)
     *   - Not a static asset path (those are file-served, not rendered)
     *   - Not a TASFA stream path (chunked streaming, must not be shared)
     *
     * The coalescing key uses the "mw:" prefix so it is distinct from the
     * per-handler keys used by the existing reqshare calls inside individual
     * handlers.  Both layers can coexist safely; the middleware layer catches
     * bursts before the handler even runs.
     * ----------------------------------------------------------------------- */
    bool mw_coalesce = false;
    char mw_key[768];
    mw_key[0] = '\0';

    if ((req->method == CWIST_HTTP_GET || req->method == CWIST_HTTP_HEAD)
            && !is_static_asset) {
        /* Check for Cookie header — authenticated / personalised responses
         * must not be shared across users. */
        const char *cookie = cwist_http_header_get(req->headers, "Cookie");
        bool has_cookie = (cookie && cookie[0]);

        /* Skip TASFA stream paths; they carry per-connection chunked state. */
        const char *pdata = (req->path && req->path->data) ? req->path->data : "";
        bool is_stream = (strncmp(pdata, "/__tasfa_stream__/", 18) == 0);

        if (!has_cookie && !is_stream) {
            const char *qdata = (req->query && req->query->data && req->query->data[0])
                                ? req->query->data : NULL;
            if (qdata) {
                snprintf(mw_key, sizeof(mw_key), "mw:GET:%s?%s", pdata, qdata);
            } else {
                snprintf(mw_key, sizeof(mw_key), "mw:GET:%s", pdata);
            }
            mw_key[sizeof(mw_key) - 1] = '\0';

            bool leader = false;
            cwist_sstring *shared = reqshare_wait_or_start(mw_key, &leader);
            if (!leader) {
                /* A concurrent request already computed this page.
                 * Serve the shared result directly without calling the handler. */
                if (shared) {
                    send_html_res(res, shared); /* takes ownership of shared */
                } else {
                    /* Leader failed (e.g. render error) — fall through to handler. */
                    mw_key[0] = '\0';
                    next(req, res);
                }
                goto mw_postprocess;
            }
            /* We are the leader: execute the handler, then broadcast result. */
            mw_coalesce = true;
        }
    }
    ...
    if (mw_coalesce && mw_key[0]) {
        /* Broadcast the rendered body to any waiters.  We only share 200 OK
         * HTML responses; errors and redirects are not coalesced. */
        if (res->status_code == CWIST_HTTP_OK
                && res->body && res->body->size > 0) {
            cwist_sstring *snapshot = cwist_sstring_create();
            if (snapshot) {
                cwist_sstring_append_len(snapshot, res->body->data, res->body->size);
                reqshare_finish(mw_key, snapshot);
                cwist_sstring_destroy(snapshot);
            } else {
                reqshare_finish(mw_key, NULL);
            }
        } else {
            reqshare_finish(mw_key, NULL);
        }

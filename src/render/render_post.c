        ...
        if (i + 2 <= len && strncmp(data + i, "<a", 2) == 0) {
            ...
            if (tag_end) {
                ...
                if (extract_download_id_from_img_tag(tag, &fid)) {
                    ...
                    const char *kind = render_file_media_kind(file);
                    if (kind[0]) {
                        // All multimedia (image, video, audio) can be upgraded from <img> tags:
                        append_inline_media_from_file(out, file, fid, kind);
...
cwist_sstring *render_post_list(cJSON *posts, cJSON *boards, bool dark, const char *user_role, int page, int total_pages, const char *board_slug, const char *search, const char *search_type, const char *profile_pic, int user_id, bool is_mobile, cJSON *children) {
    ...
    cwist_sstring_append(b, "' method='get' class='search-form'>");
    cwist_sstring_append(b, "<div class='search-label-row'>");
    cwist_sstring_append(b, "<span class='search-label-text'>");
    ...
    cwist_sstring_append(b, "</div>");
    cwist_sstring_append(b, "<div class='search-cluster'>");
    ...
    cwist_sstring_append(b, "<select name='search_type' class='search-adv-select'>");
...
cwist_sstring *render_post_detail(cJSON *post, cJSON *files, cJSON *comments, bool dark, const char *user_role, bool pqc_verified, int vote_up, int vote_down, int user_vote, const char *profile_pic, const char *author_profile_pic, int user_id, const char *ephemeral_delete_pin, bool is_mobile) {
    ...
    cwist_sstring_append(b, "<select id='post-translate-target' class='btn btn-outline post-translate-target' style='padding:6px 8px;font-size:13px' aria-label='Translation target language' title='Translation target language'><option value='en'>English</option><option value='ko'>Korean</option><option value='ja'>Japanese</option><option value='zh'>Chinese (Simplified)</option><option value='es'>Spanish</option><option value='fr'>French</option><option value='de'>German</option><option value='pt'>Portuguese</option><option value='ru'>Russian</option><option value='ar'>Arabic</option><option value='hi'>Hindi</option><option value='id'>Indonesian</option><option value='vi'>Vietnamese</option></select>");
    ...
    cwist_sstring_append(b, "</div>");
    cwist_sstring_append(b, "<script src='/assets/js/post-translate.js?v=8' defer></script>");
    ...
    if (files && cJSON_GetArraySize(files) > 0) {
        ...
        if (valid_files > 0) {
            cwist_sstring_append(b, "<h3 style='margin-top:40px;margin-bottom:16px'>Attachments</h3>");
            cwist_sstring_append(b, "<div class='post-attachments' style='display:flex;flex-direction:column;gap:24px;max-width:760px;margin:0 auto'>");
            for (int i = 0; i < n; i++) {
                cJSON *f = cJSON_GetArrayItem(files, i);
                cJSON *fname = cJSON_GetObjectItem(f, "filename");

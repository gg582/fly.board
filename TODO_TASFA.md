...
- [ ] Parallelism floor on good 4G/5G links: the current floor is mobile `4` and desktop `8`. Verify whether this is too aggressive on low-end devices or under thermal pressure.
...
- [ ] SIMD: HTP line-sum has a SIMD path plus simple scalar fallback. Because Aitken/Wynn sample counts are small, SIMD call overhead may outweigh gains, so SIMD expansion beyond HTP verification is deferred until benchmarked.

...
|-----------|----------|--------------|------|
| `/` (홈) | **729.79** | 7.29ms | DB 쿼리 + 마크다운 렌더링 |
| `/login` | **768.09** | 48.66ms | 정적 폼 (캐시 가능) |
| `/boards` | **681.60** | 124.29ms | DB 기반 목록 |

### 리소스 사용량 (최고 부하 시)

| 항목 | 값 |
|------|-----|
| CPU 사용률 | 약 140% (4코어 기준) |
| RAM 점유 (RSS) | 약 11.5 MB |
| 가상 메모리 (VSZ) | 약 1.1 GB |

> 참고: 본 벤치마크는 동시 요청을 직렬화(`pthread_mutex_t`)한 상태에서 수행되었습니다.  
> `ulimit -n`이 1024로 제한되어 있어 400 connections 이상은 `TIME_WAIT` 누적으로 인해 연결 거부가 발생할 수 있습니다.

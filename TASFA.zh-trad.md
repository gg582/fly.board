...
上傳客戶端測量區塊完成時間、重試、逾時，以及可用時的 Network Information API 提示。它將這些輸入傳送到 `/file/upload/init` 和 `/file/upload/renegotiate`；伺服器傳回目前並行視窗和最大視窗。乾淨的完成會迅速將活動視窗提升到協商的最大值，而失敗則一次減少一個槽位。看門狗會中止停滯的上傳並從伺服器位元圖中恢復，而不是讓傳輸卡住。
...
`S_hat` 被限制在 `[0, 1]` 範圍內，僅用作下一個區塊的防禦信號。預測品質高時保持正常路徑，並讓自適應視窗增長。中等偏低時標記下一個區塊為 `guarded`，在分發前將活動並行視窗減少一格。非常低時將下一個上傳區塊標記為串列 AES-GCM 回退，或在發出下一個範圍群組請求前減少下載的 `span` 和並行度。這是前向的擁塞/失敗防禦，而非密碼學預言機。

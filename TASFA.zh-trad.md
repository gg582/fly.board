...
用戶端將 `chunk_size` 視為工作階段契約，因為它定義了每個分塊的檔案偏移。在活躍上傳期間，它調整並行和重試行為，同時學習到的分塊大小提示將套用於下一個 TASFA 工作階段：良好鏈路將提示 `*2` 提高，降級鏈路 `/2` 降低。
...
### 混合 HTP-XOR 糾刪碼（Erasure Coding）

為了解決低配備伺服器上高運算成本的封包/分塊遺失問題，TASFA 使用了 HTP 拓撲嫌疑定位與輕量化 XOR 糾刪碼相結合的混合模型：
- **XOR 奇偶校驗生成**：每個 6 分塊組都包含第 7 個奇偶校驗分塊 $P = C_0 \oplus C_1 \oplus C_2 \oplus C_3 \oplus C_4 \oplus C_5$。
- **零成本嫌疑定位**：當某個組完整性校驗失敗時，HTP 的線和與嫌疑評分能以極低的 CPU 成本（簡單的純量算術）精準定位損壞或遺失的具體分塊索引 $i$。
- **$O(1)$ 復原運算**：若一組中恰好只有一個分塊 $C_i$ 遺失或損壞，伺服器會立即透過 $C_i = P \oplus \bigoplus_{j \neq i} C_j$ 公式，使用 SIMD 位元 XOR 運算完成重構。這避免了昂貴的矩陣求逆和伽羅瓦域（Galois Field）運算。
- **多重遺失退避**：若有兩個或更多分塊損壞或遺失，伺服器將繞過 FEC 限制，並退避至透過 `retry_targets` 進行的常規重傳（ARQ）。

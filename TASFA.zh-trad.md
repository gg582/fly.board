...
`S_hat` 被限制在 `[0, 1]` 範圍內，僅用作下一個區塊的防禦信號。預測品質高時保持正常路徑，並讓自適應視窗增長。中等偏低時標記下一個區塊為 `guarded`，在分發前將活動並行視窗減少一格。非常低時將下一個上傳區塊標記為並行處理 AES-GCM 回退，或在發出下一個範圍群組請求前減少下載的 `span` 和並行度。這是前向的擁塞/失敗防禦，而非密碼學預言機。

...
|---|---|---|---|
| Idle → C10k | +62.22 MB | 10,000 | ~6.4 KB / 連線 |
| C10k → C1m | +492 KB | 990,000 | ~0.5 byte / 新增連線 |

從 Idle 到 C10k 的初始躍升預先支付了 TLS 狀態、連線緩衝區與 worker 開銷。此後，再增加 990,000 條連線，每條連線的 RSS 成本還不到半個位元組 —— 每條連線的記憶體成本實際上是平坦的。

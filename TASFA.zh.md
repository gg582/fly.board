...
客户端将 `chunk_size` 视为会话契约，因为它定义了每个分块的文件偏移。在活跃上传期间，它调整并发和重试行为，同时学习到的分块大小提示将应用于下一个 TASFA 会话：良好链路将提示 `*2` 提高，降级链路 `/2` 降低。
...
### 混合 HTP-XOR 纠删码（Erasure Coding）

为了解决低配置服务器上高计算成本的数据包/分块丢失问题，TASFA 使用了 HTP 拓扑嫌疑定位与轻量化 XOR 纠删码相结合的混合模型：
- **XOR 奇偶校验生成**：每个 6 分块组都包含第 7 个奇偶校验分块 $P = C_0 \oplus C_1 \oplus C_2 \oplus C_3 \oplus C_4 \oplus C_5$。
- **零成本嫌疑定位**：当某个组完整性校验失败时，HTP 的线和与嫌疑评分能以极低的 CPU 成本（简单的标量算术）精准定位损坏或丢失的具体分块索引 $i$。
- **$O(1)$ 恢复运算**：若一组中恰好只有一个分块 $C_i$ 丢失或损坏，服务器会立即通过 $C_i = P \oplus \bigoplus_{j \neq i} C_j$ 公式，使用 SIMD 位元 XOR 运算完成重构。这避免了昂贵的矩阵求逆和伽罗瓦域（Galois Field）运算。
- **多重丢失退避**：若有两个或更多分块损坏或丢失，服务器将绕过 FEC 限制，并退避至通过 `retry_targets` 进行的常规重传（ARQ）。
...
- 下载合并（span 组大小）：成功时 `*1.15`，失败时 `*0.85`，最多 `16` 个分块
- 上传 xhr 超时：至少 `180 s`
- 上传会话 fetch 超时：`30 s`
...
上传客户端测量块完成时间、重试、超时，以及可用时的 Network Information API 提示。它将这些输入发送到 `/file/upload/init` 和 `/file/upload/renegotiate`；服务器返回当前并行窗口和最大窗口。干净的完成会将活动窗口以 `*1.15` 提升到协商的最大值，而暂时性失败以 `*0.85` 降低。AES-GCM 回退也在自适应窗口内运行。

每个分块设置绝对超时，以防止连接接收部分数据但永不关闭导致的停滞。如果看门狗检测到上传停滞（超时内无分块完成），客户端将执行**激进重启（aggressive restart）**：将 `targetParallel` 重置为 `maxParallel`，将 `dispatchPacingMs` 设为 `0`，然后从服务器位图恢复上传。学习到的分块大小提示仅在**会话本地**保持，不会立即持久化到 `localStorage`；仅应用于下一个 TASFA 会话。

...
> 为数不多的简单博客引擎之一，在连接规模扩大时内存几乎保持平稳：空闲时 **~104 MB RSS**（4 个 worker；在实际生产服务器上以单个 worker 运行时可维持在 **68–120 MB**），在 C10k、C100k 乃至 C1m 下仍约为 **~110–146 MB**。
> 基于 C 语言 CWIST Web 框架的轻量级论坛兼博客引擎，支持 HTTPS/3、Argon2id、PQC 签名与 NATS 消息。

## 特性

- **内存高效且连接可扩展** – 栈+堆 C 实现。空闲时 **~104 MB RSS**；从 C10k 到 C1m 并发连接，RSS 始终保持在 **~110–146 MB** 左右。
...
| 状态 | RSS | 较上次变化 | 备注 |
|-------|-----|-----------------|-------|
| 空闲 | **~104 MB** (106,192 KB) | — | 4 个 worker，无连接 |
| C10k | **~110 MB** (112,436 KB) | +6,244 KB | 10,000 并发连接 |
...
|---|---|---|---|
| Idle → C10k | +6,244 KB | 10,000 | ~0.6 KB / 连接 |

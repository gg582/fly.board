...
3. Downloads chunks **sequentially** starting from index 0 with `span=1`. Each decrypted chunk is pushed into the SW stream via `TASFA_STREAM_CHUNK`.
...
5. Remaining chunks continue to download in the background and are fed into the stream. When the final chunk is pushed, the client sends `TASFA_STREAM_CLOSE` so the browser sees an EOF.
